      ...
      <div className="container mx-auto px-[var(--card-padding,1.25rem)]">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative">
            ...
            <div className="space-y-[var(--content-density-gap,1rem)] md:space-y-12">
              {steps.map((step, index) => (
                <div
                  key={step.id}
                  className="relative flex items-start gap-[var(--content-density-gap,1rem)]"
                  data-testid={`process-step-${step.id}`}
                >
                  <div className="flex-shrink-0 relative z-10">
                    <div className="w-16 h-16 rounded-full bg-primary-600 text-white dark:bg-primary-400 dark:text-black flex items-center justify-center text-xl font-bold shadow-[var(--card-shadow)]">
                      {step.number || index + 1}
                    </div>
                  </div>
