...
function FeatureCard({ icon, title, description, index = 0 }: { icon: string; title: string; description: string; index?: number }) {
  ...
  const bgIdx = index % 3
  return (
    <div className="p-6 rounded-lg border bg-card text-card-foreground" data-testid={`card-feature-${title.toLowerCase().replace(/\s+/g, '-')}`}>
...
function TestimonialCard({ name, role, company, quote, avatarUrl, companyLogoUrl }: { name: string; role: string; company: string; quote: string; avatarUrl?: string; companyLogoUrl?: string }) {
  ...
  return (
    <div className="p-6 rounded-lg border bg-card text-card-foreground relative" data-testid={`card-testimonial-${name.toLowerCase().replace(/\s+/g, '-')}`}>
