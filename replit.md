...
- **Dashboard Enhancements (Phase 6)**: Admin CRM card (click affiliate → full profile drawer with earnings/payouts/tickets/activities/notes), revenue attribution report (affiliate vs direct split), bulk payout processing with auto receipt emails, affiliate health scores (green/yellow/red), quick notes. Affiliate earnings statements (PDF), portfolio view (referrals as investments with LTV), commission lifecycle tracker (7-step stepper), contract view with signing, tax summary (1099-ready), campaign creator with UTM. User billing page (invoice history + subscription management + Stripe portal), support ticket submission/history, account security (password change, 2FA placeholder), affiliate invitation prompt.
