...
**CSS Variable Pipeline (Server-Side Injection):** Design system CSS variables are injected server-side in the root layout (`src/app/layout.tsx`) via a `<style id="design-system-vars">` tag. The pipeline is: `getSettingsServer()` (fetches from Supabase) → `computeDesignSystemCSS()` (pure function, computes all vars) → injected in `<head>`. Data attributes are also applied server-side on the `<html>` tag. Client-side `ThemeSettingsProvider` acts as a safety net for dark/light mode switching and palette page live preview. Key files: `src/lib/get-settings-server.ts`, `src/lib/compute-css-vars.ts`.
