...
1. **Read `docs/PRODUCT_IDENTITY.md`** — Understand what PassivePost actually is. The 217-feature vision is intentional. Never suggest cutting features or simplifying the affiliate program.
2. **Read `docs/DESIGN_SYSTEM_RULES.md`** — MANDATORY before any component or UI work. Contains CSS variable mapping, DS wrapper components (`DSCard`, `DSGrid`, `DSSection`), and Never/Always rules.
3. **Read `docs/FEATURE_INVENTORY.md`** — Know what's already built. Search for keywords related to what you're about to build. If something similar exists, EXTEND it.
4. **Read `docs/LESSONS_LEARNED.md`** — Know the anti-patterns and technical gotchas.
5. **Read `docs/ROADMAP.md`** — Know what's in progress and what's next.
6. **Plan with integration requirements** — For every feature, list which existing systems it connects to.
7. **Update Session Log** in ROADMAP.md at end of every session.
