...
| **Palette-Aware Color Audit** | Zero hardcoded Tailwind color classes (`text-red-600`, `bg-blue-500`, etc.) across all pages. Zero hardcoded spacing classes (`p-6`, `gap-4`, `rounded-lg`, `shadow-sm`) across 113+ non-UI-primitive files — all use CSS variables with fallbacks. | Admin color and spacing changes propagate everywhere — no orphaned hardcoded values that ignore the palette. |
| **Typography Font Selectors** | Heading font and body font selectors on the palette page with 20 curated Google Fonts (Inter, DM Sans, Poppins, Playfair Display, etc.). Changes reflected reactively in the live preview. | Admins can choose brand-appropriate fonts without touching code. |
| **Typography Color Pickers (Light/Dark)** | Four color pickers on the palette page: Heading Color (Light), Heading Color (Dark), Body Color (Light), Body Color (Dark), each with clear/reset capability. Injected as `--heading-color` and `--body-color` CSS variables based on the current mode. Live Preview reflects the correct mode's colors when toggling light/dark. Backward-compatible with legacy `headingColor`/`bodyColor` settings. | Admins can set exact text colors per mode — different heading/body colors for light and dark themes. |
...
| **Admin Design System UI** | 16 collapsible accordion sections in the palette page covering every design property. Live previews for typography (with reactive font + color changes) and skeleton animations. Visual toast position picker. Font family selectors and color pickers for both headings and body text. | Admin-friendly interface — no technical knowledge needed to customize the design. |
