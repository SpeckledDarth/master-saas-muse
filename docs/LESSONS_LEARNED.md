...
| Leaving shadcn UI primitives with hardcoded `rounded-*` classes | Textarea, Select, Dialog, Tabs ignore palette radius settings — half the form/modal elements on the site don't respond to the design system | Use inline `style={{ borderRadius: 'var(--input-radius, 0.75rem)' }}` (for inputs) or `var(--card-radius, 0.75rem)` (for containers) or `var(--btn-radius, 9999px)` (for interactive triggers). Pattern matches button.tsx and input.tsx. Exempt: tiny close buttons (`rounded-sm` on dialog/sheet close X). |
| Hardcoding `--sidebar-width` in layout inline styles | Palette Sidebar Width control (narrow/default/wide) has no effect — layouts override it with `16rem` | Remove `--sidebar-width` from layout inline styles. Let it inherit from (1) palette JS resolver or (2) globals.css default. Only `--sidebar-width-icon` should be in the inline style. |
| Missing CSS variable defaults in globals.css `:root` | Before JavaScript loads, components render with no radius/spacing, causing layout flash | Every CSS variable used in component `style={}` props must have a default in globals.css `:root`. Check: `--btn-radius`, `--card-radius`, `--input-radius`, `--badge-radius`, `--card-padding`, `--card-shadow`, etc. |
