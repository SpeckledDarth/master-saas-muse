...
| 3.6 | Affiliate Enhancements & Discount Codes | Sprints 1-3 Complete + Audit System + Bug Fixes done — Sprint 4 not started (13/32 features remaining) | Week 4-6 |
...
- Remaining Sprint 3-4 APIs: messages/, webhooks/ (to be created)
- Column-resilient pattern: All affiliate API routes (broadcasts, payouts, contests, applications, audit) use a "try full insert → catch column error (42703) → retry with minimal required fields" pattern. This ensures APIs work regardless of which optional columns exist in Supabase.
